...
import com.lin.request.req.UploadPluginReq;
...
public class PluginAction {
    ...
    @Subscribe
    @RequestMapper(url = "/block-server/uploadPlugin", codecName = "blockMutipartDecodec", clazz = UploadPluginReq.class)
    public void uploadPlugin(UploadPluginReq req) {
        CommonResp resp = new CommonResp();
        try {
			pluginService.savePlugin(req.getFileByte(),req.getName(),req.getFileName(),"");
        } catch (Exception e) {
            resp.setResult(Result.ERROR_SYSTEM);
            resp.setMsg(Result.getMsg(Result.ERROR_SYSTEM));
        } finally {
